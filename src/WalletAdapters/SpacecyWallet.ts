...
import { AptosAccount, MaybeHexString, Types, TxnBuilderTypes, BCS } from 'aptos';
import {
...
export class SpacecyWalletAdapter extends BaseWalletAdapter {
    ...
    async checkIsConnectedAndAccount(): Promise<{ isConnected: boolean, accountWallet: MaybeHexString }> {
        try {
            const provider = this._provider || window.spacecy;
